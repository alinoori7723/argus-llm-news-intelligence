...
const SOURCE_ID = 'fed_press_all';
const RECORDING_TOOL_VERSION = 'record-fed-rss-v1';
...
const LEGAL_EVIDENCE_NOTE =
  ...
  'no automatic polling. https://www.federalreserve.gov/feeds/feeds.htm';
...
const xmlPath = resolve(outDir, 'press_all.rss.xml');
const metaPath = resolve(outDir, 'press_all.meta.json');

const main = async (): Promise<void> => {
  const observedAt = new Date().toISOString();
  console.log(`[record-fed-rss] GET ${FEED_URL}`);
  ...
  const meta = {
    ...
    legalEvidenceNote: LEGAL_EVIDENCE_NOTE,
  ...
  console.log(`[record-fed-rss] wrote ${xmlPath} (${byteLength} bytes)`);
  console.log(`[record-fed-rss] sha256 ${sha256}`);
  console.log(`[record-fed-rss] wrote ${metaPath}`);
};

main().catch((err) => {
  console.error(`[record-fed-rss] FAILED: ${(err as Error).message}`);
  process.exitCode = 1;
});
